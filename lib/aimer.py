...
class Aimer:
    ...
    def start(self):
        ...
        while 1:
            ...
            if cdll.user32.GetAsyncKeyState(0x71) & 0x8000:
                self.trigger = None
                # f2关闭自瞄
            if cdll.user32.GetAsyncKeyState(0x70) & 0x8000:
                ...
                if cdll.user32.GetAsyncKeyState(self.trigger) & 0x8000:
                    ...
                    for Soldier in data.soldiers:
                        if self.lastSoldier == Soldier.ptr:
                            # print(Soldier.name)
                            # 循环锁的对手名字
                            # if Soldier.name != self.black_name:
                            #     continue
